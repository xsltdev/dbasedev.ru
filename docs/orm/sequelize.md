...
console.log(
    'Таблица для модели `User` только что была создана заново!'
);
```
...
const incrementResult = await john.increment('age', {
    by: 2,
});
...
await User.bulkCreate([{ name: 'John' }, { name: 'J' }], {
    validate: true,
}); // Ошибка!
...
const Post = sequelize.define('post', {
    content: {
        type: DataTypes.TEXT,
        get() {
            const storedValue = this.getDataValue(
                'content'
            );
            ...
            const unzippedBuffer = gunzipSync(
                gzippedBuffer
            );
        ...
        validate: {
            customValidator(value) {
                if (value === null && this.age < 18) {
                    throw new Error(
                        'Нулевые значения разрешены только совершеннолетним!'
                    );
                }
            },
        },
...
console.log(
    (await Ship.findAll({ include: Captain })).toJSON()
);
// или
console.log(
    (await Ship.findAll({ include: 'Captain' })).toJSON()
...
console.log(
    (await Ship.findAll({ include: 'leader' })).toJSON()
);
```
...
class Comment extends Model {
    getCommentable(options) {
        ...
        const mixinMethodName = `get${capitilize(
...
```js
await Project.scope('random', {
    method: ['accessLevel', 10],
});
```
...
const Qux = sequelize.define('Qux', {
    name: DataTypes.STRING,
});
...
```js
const createPostWithReactions = async (
    content,
    reactionTypes
) => {
...
-   [Sequelize](https://my-js.org/docs/guide/sequelize)
